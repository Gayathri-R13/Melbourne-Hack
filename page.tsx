...
const Confetti = ({ show, width, height }) => {
  ...
  const confettiPieces = Array.from({ length: 300 }).map((_, i) => {
    ...
    const style = {
      ...
      // Dynamically set the final position of the confetti piece
    ...
    {[...Array(100)].map((_, i) => (
      <span
        ...
        style={{
          ...
          animationDuration: `${2 + Math.random() * 3}s`, // different fall speeds
          animationDelay: `${Math.random() * 5}s`, // staggered starts
        }}
      >
        🍀
      </span>
...
const avatars = [
  ...
  { id: 2, name: "Green Guru", image_url: "https://i.pinimg.com/736x/94/e0/90/94e090a659640666db4cb3267bf62192.jpg" },
  {
    ...
    image_url: "https://i.pinimg.com/736x/e3/ed/70/e3ed705a87b56a391326564d8cf9947c.jpg",
  },
  {
    id: 4,
    name: "Forest Friend",
    image_url: "https://i.pinimg.com/736x/58/a7/3c/58a73c4f1a7bb444eaa2c44de147f7ff.jpg",
  },
  { id: 5, name: "Eco-Explorer", image_url: "https://i.pinimg.com/736x/e3/2c/da/e32cda89df20e5168125d0373b5a0d0d.jpg" },
  {
    id: 6,
    name: "Plant Pioneer",
    image_url: "https://i.pinimg.com/736x/9e/4d/d2/9e4dd2cbb97f3fec0d9e7c9b31c6f335.jpg",
  },
  {
    id: 7,
    name: "Recycle Ranger",
    image_url: "https://i.pinimg.com/736x/63/9b/79/639b79dc522f1fbde8999ae0c549a5ae.jpg",
  },
  {
    id: 8,
    name: "Green Guardian",
    image_url: "https://i.pinimg.com/736x/24/2e/b2/242eb23bc0ee187a742bf71ddf298ed9.jpg",
  },
]
...
export default function EcoFactoryApp() {
  ...
  // --- Game State ---
  const [activeActivityGame, setActiveActivityGame] = useState(null) // 'recycling', 'carbon'
  ...
  const handleGameEnd = (finalScore, finalCo2, gameName) => {
    ...
    showMessageBox(
      `${gameName} Complete!`,
      `Your final score was ${Math.round(finalScore)}. Your sustainability score increased by ${scoreIncrease.toFixed(1)}!`,
    )
  ...
  const endRecyclingGame = () => {
    ...
    if (recyclingGameScore > 0) {
      showMessageBox(
        "Recycling Game Complete!",
        `You scored ${recyclingGameScore} points! Your sustainability score increased by ${scoreIncrease.toFixed(1)}!`,
      )
      setChatMessages((prev) => [
        ...prev,
        {
  ...
  const endCarbonGame = () => {
    ...
    showMessageBox(
      ...
      `Based on your choices, you scored ${carbonGameScore} points. Your sustainability score increased by ${scoreIncrease.toFixed(1)}!`,
    )
    setChatMessages((prev) => [
      ...prev,
      {
  ...
  const sendMessage = () => {
    ...
    // Directly display the error message without making an API call
    setTimeout(() => {
      setChatMessages((prev) => [
        ...prev,
        {
  ...
  const handleDecision = (co2Reduction: number, recyclingIncrease: number, actionCost: number) => {
  ...
  const nextLevelProgress = nextLevel
    ? ((sustainabilityScore - currentLevel.scoreThreshold) / (nextLevel.scoreThreshold - currentLevel.scoreThreshold)) *
      100
    : 100
        ...
        {!isGameFinished ? (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <Badge>
                Question {carbonGameStep + 1} of {carbonScenarios.length}
              </Badge>
            ...
            <div className="grid grid-cols-1 gap-2">
              {currentScenario.options.map((option) => (
                <Button
                  key={option.text}
                  variant="outline"
                  className="w-full text-left justify-start whitespace-normal h-auto py-2 bg-transparent"
          ...
          <div className="text-center space-y-4">
            ...
            <p className="text-sm">
              Based on your choices, you earned a score of{" "}
              <span className="text-lg font-bold text-primary">{carbonGameScore}</span> points.
            </p>
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-2">
            <Card className="p-3 bg-muted/50">
              ...
              <CardContent className="p-0 mt-1 text-lg font-bold text-green-600">{co2Reduction}%</CardContent>
            </Card>
            <Card className="p-3 bg-muted/50">
              <CardTitle className="text-sm text-muted-foreground">Recycling Rate</CardTitle>
              <CardContent className="p-0 mt-1 text-lg font-bold text-blue-600">{recyclingRate}%</CardContent>
            </Card>
          </div>
        </div>
      </div>
  ...
  return (
    ...
    >
      ...
      {showConfetti && (<Confetti show={showConfetti} width={width} height={height}/>)}
      {levelUpMessage && (
                    ...
                    <CardContent className="space-y-2">
                      {stageOptions[stage].map((option) => (
                        <Button
                          key={option.id}
                          variant="outline"
                          className="h-auto w-full flex flex-col gap-1 py-4 bg-transparent"
                          onClick={() => {
              ...
              <TabsContent value="progress" className="space-y-4 mt-4">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-3">
                    <span
                      className={`text-sm ${dailyChallenge.completed ? "line-through text-muted-foreground" : "text-foreground"}`}
                    >
                      {dailyChallenge.text}
                    </span>
                    <Button
                      onClick={() => handleDailyChallenge(dailyChallenge.text)}
